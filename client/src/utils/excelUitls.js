...
const headerColumns = [
  ...
  { header: "Is Draft?", key: "draft", width: 15 },
];
...
export const downloadExcelTemplate = async (setProgress) => {
  try {
    ...
    worksheet.getCell("A1").note =
      "Upload an image directly in this column. Do not enter a Image URL Link!.";
    ...
    worksheet.getCell("K1").note = "Active or Draft.";
    ...
    for (let i = 2; i <= 101; i++) {
      ...
      worksheet.getCell(`K${i}`).dataValidation = {
        ...
        showErrorMessage: true,
        errorTitle: "Invalid draft status",
        error: "Please select a valid draft item from the dropdown list.",
      };
    }
    ...
    reader.onload = async (e) => {
      ...
      workbook.worksheets.forEach((sheet) => {
        sheet.getImages().forEach((img) => {
          ...
          if (image) {
            const base64Image = `data:image/${image.extension};base64,${image.buffer.toString(
              "base64"
            )}`;
      ...
      worksheet.eachRow((row, rowNumber) => {
        if (rowNumber > 1) {
          const addData = {
            ...
            draft: row.getCell(11).value.toLowerCase() === "draft",
          };
          ...
          if (
            !addData.asset_name ||
            addData.asset_name.length < 3 ||
            addData.asset_name.length > 80
          ) {
            toast.error(
              `Row ${rowNumber}: Item name is required and must not less than 3 or exceed 80 characters.`,
          ...
          if (!addData.serial_number || addData.serial_number.length > 15) {
            toast.error(
              `Row ${rowNumber}: Serial number is required and must not exceed 15 characters.`,
              { position: "bottom-right" }
            );
            hasError = true;
            return;
