...
const LoanDetailByUserComponent = ({
  ...
  const expiryDate = loanData?.pickup_time
    ? new Date(new Date(loanData.pickup_time).getTime() + 3 * 24 * 60 * 60 * 1000) // 3 days
    : null;
    ...
    <>
      <Card>
        <div className="flex flex-col justify-center items-center w-full gap-2 mb-2">
          ...
          {loanData?.loan_status === "Borrowed" && !loanData?.returned_confirmed_date_by_staff && (
            <div className="font-semibold text-xs w-full text-center px-3 py-2 rounded-lg bg-green-100/20 border border-green-800">
              <p className="text-green-800 italic">
                Please meet or contact our staff if you want to return the borrowed item, or if you
                have any questions you can contact our staff.
              </p>
            </div>
          )}
          ...
          {meetingInfoByLoanId?.status === "Approved" && !loanData?.borrow_confirmed_by && (
          ...
          {loanData?.loan_status === "Ready to Pickup" && !meetingInfoByLoanId && (
            <Button
          ...
          {!loanData?.borrow_confirmed_date_by_user && loanData?.borrow_confirmed_by && (
            <Button
          ...
          {(loanData?.loan_status === "Borrowed" ||
            ...
            !loanData?.returned_confirmed_date_by_user && (
              <Button
