...
export const getAllUsersInfor = createAsyncThunk(
  "user/all_users",
  async ({ token, page, sort, program, search, all }, thunkAPI) => {
    try {
      const params = {
...
export const addUserByAdmin = createAsyncThunk("user/add_user_by_admin", async (data, thunkAPI) => {
  try {
    const token = await tokenService.accessToken(data);

    return await userService.addUserByAdmin(data, token);
  } catch (error) {
    const message =
      (error.response && error.response.data && error.response.data.message) ||
      error.message ||
