...
export const createMeeting = async (req, res) => {
  try {
    ...
    if (existingMeeting) {
      return res
        .status(400)
        .json({ message: "A meeting request has already been created for this loan transaction." });
    }
    ...
    if (loanTransaction.loan_status !== "Ready to Pickup") {
      return res
        .status(404)
        .json({ message: "Invalid loan transaction status for meeting request" });
    }
    ...
    if (meetingDay === 0 || meetingDay === 6) {
      return res
        .status(400)
        .json({ message: "Meeting date must be on a weekday (Monday - Friday)." });
    }
    ...
    if (formattedMeetingDate > expectedReturnDate) {
      return res
        .status(400)
        .json({ message: "Meeting date cannot exceed the expected return date." });
    }
    ...
    await createNotification(
      ...
      loanTransaction._id,
      `Meeting request for transaction ID: ${loanTransaction.transaction_id} has been successfully created. Please wait, your meeting request is currently being reviewed by our staff.`
    );
...
export const approveMeeting = async (req, res) => {
  try {
    ...
    await createNotification(
      ...
      `Your meeting request (Transaction ID: ${
        meeting.loanTransaction_id.transaction_id
      }) has been approved. Please meet the staff on ${meeting.meeting_date.toDateString()} at ${
    ...
    const emailText = `Your meeting request has been approved for transaction ID: ${
      meeting.loanTransaction_id.transaction_id
    }. Please meet the staff on ${meeting.meeting_date.toDateString()} at ${meeting.meeting_time}.`;
